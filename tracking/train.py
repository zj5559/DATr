...
def parse_args():
    ...
    parser.add_argument('--datr', type=int, choices=[0, 1,2], default=0)  # whether to use our data augmentation datr
...
def main():
    args = parse_args()
    if args.mode == "single":
        train_cmd = "python lib/train/run_training.py --script %s --config %s --save_dir %s --seed %d --use_lmdb %d " \
                    "--script_prv %s --config_prv %s --distill %d --datr %d --script_teacher %s --config_teacher %s --use_wandb %d"\
                    % (args.script, args.config, args.save_dir,args.seed, args.use_lmdb, args.script_prv, args.config_prv,
                       args.distill,args.datr, args.script_teacher, args.config_teacher, args.use_wandb)
    elif args.mode == "multiple":
        train_cmd = "python -m torch.distributed.launch --nproc_per_node %d --master_port %d lib/train/run_training.py " \
                    "--script %s --config %s --save_dir %s --seed %d --use_lmdb %d --script_prv %s --config_prv %s --use_wandb %d " \
                    "--distill %d --datr %d --script_teacher %s --config_teacher %s" \
                    % (args.nproc_per_node, random.randint(10000, 50000), args.script, args.config, args.save_dir,args.seed, args.use_lmdb, args.script_prv, args.config_prv, args.use_wandb,
                       args.distill,args.datr, args.script_teacher, args.config_teacher)
