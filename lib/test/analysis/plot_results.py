...
def get_auc_curve_seq(ave_success_rate_plot_overlap, valid_sequence):
    ave_success_rate_plot_overlap = ave_success_rate_plot_overlap[valid_sequence, :, :]
    auc_curve = ave_success_rate_plot_overlap * 100.0
    auc = auc_curve.mean(-1)

    return auc_curve, auc.numpy()
...
def print_results(trackers, dataset, report_name, merge_results=False,seq_eval=False,
    ...
    if seq_eval:
        ...
        for tracker in trackers:
            sheet.write(0,col,tracker.display_name)
            col+=1
    ...
    if 'success' in plot_types:
        ...
        ave_success_rate_plot_overlap = torch.tensor(eval_data['ave_success_rate_plot_overlap'])
        if seq_eval:
            auc_curve_seq, auc_seq = get_auc_curve_seq(ave_success_rate_plot_overlap, valid_sequence)
            for row in range(auc_seq.shape[0]):
                sheet.write(row+1,0,dataset[row].name)
                for col in range(auc_seq.shape[1]):
                    sheet.write(row+1,col+1,round(float(auc_seq[row][col]),2))
            print(os.path.join(env_settings().got_reports_path, 'results_'+dataset[0].dataset+'.xls'))
            book.save(os.path.join(env_settings().got_reports_path,'results_'+dataset[0].dataset+'.xls'))
        # Index out valid sequences
        auc_curve, auc = get_auc_curve(ave_success_rate_plot_overlap, valid_sequence)
        scores['AUC'] = auc
