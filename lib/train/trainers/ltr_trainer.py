...
class LTRTrainer(BaseTrainer):
    def __init__(self, actor, loaders, optimizer, settings, lr_scheduler=None,imix=False,imix_epoch=40,imix_interval=1, use_amp=False):
        """
        args:
        ...
        self.imix=imix
        self.imix_epoch=imix_epoch
        self.imix_interval=imix_interval
    ...
    def train_epoch(self):
        ...
        for loader in self.loaders:
            if loader.name=='train' and self.imix and self.epoch>self.imix_epoch and self.epoch % self.imix_interval ==0:
                if isinstance(loader.sampler,torch.utils.data.sampler.RandomSampler):
                    loader.sampler.data_source.cutmix_prob = self.imix
                else:
                    loader.sampler.dataset.cutmix_prob=self.imix
            else:
                if isinstance(loader.sampler,torch.utils.data.sampler.RandomSampler):
                    loader.sampler.data_source.cutmix_prob = False
                else:
                    loader.sampler.dataset.cutmix_prob=False
                # loader.sampler.data_source.cutmix_prob = False
