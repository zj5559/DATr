...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
                 ...
                 train_cls=False, pos_prob=0.5,cutmix_prob=False,max_mix_obj=1):
        """
        args:
    ...
    def sample_seq_with_same_cls(self, dataset, cls, is_video_dataset):
        ...
        while not enough_visible_frames:
            ...
            enough_visible_frames = visible.type(torch.int64).sum().item() > 2 * self.num_search_frames and len(visible) >= 20
    ...
    def __getitem__(self, index):
        if self.train_cls:
            ...
            return self.getitem()
    def get_ts_frame_ids(self,is_video_dataset,visible, seq_info_dict):
        if is_video_dataset:
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            template_frame_ids, search_frame_ids=self.get_ts_frame_ids(is_video_dataset,visible, seq_info_dict)
            try:
                ...
                data = TensorDict({'template_images': template_frames,
                                   ...
                                   'is_cutmix': False})
                if self.cutmix_prob:
                    seq_id_extra, visible_extra, seq_info_dict_extra = self.sample_seq_with_same_cls(dataset,
                                                                                                     meta_obj_test.get(
                                                                                                         'object_class_name'),
                                                                                                     is_video_dataset)
                    ct=0
                    while seq_id_extra==seq_id:
                        ...
                        seq_id_extra, visible_extra, seq_info_dict_extra = self.sample_seq_with_same_cls(
                            ...
                            is_video_dataset)
                        ct+=1
                    template_frame_ids_extra, search_frame_ids_extra = self.get_ts_frame_ids(is_video_dataset, visible_extra,
                                                                                 seq_info_dict_extra)
                    template_frames_extra, template_anno_extra, meta_obj_train_extra = dataset.get_frames(seq_id_extra, template_frame_ids_extra,
                                                                                        seq_info_dict_extra)
                    search_frames_extra, search_anno_extra, meta_obj_test_extra = dataset.get_frames(seq_id_extra, search_frame_ids_extra,
                                                                                   seq_info_dict_extra)

                    H, W, _ = template_frames_extra[0].shape
                    template_masks_extra = template_anno_extra['mask'] if 'mask' in template_anno_extra else [torch.zeros(
                        (H, W))] * self.num_template_frames
                    search_masks_extra = search_anno_extra['mask'] if 'mask' in search_anno_extra else [torch.zeros(
                        (H, W))] * self.num_search_frames

                    data['is_cutmix'] = True
                    data['template_images_extra'] = template_frames_extra
                    data['template_anno_extra'] = template_anno_extra['bbox']
                    data['template_masks_extra'] = template_masks_extra
                    data['search_images_extra'] = search_frames_extra
                    data['search_anno_extra'] = search_anno_extra['bbox']
                    data['search_masks_extra'] = search_masks_extra
