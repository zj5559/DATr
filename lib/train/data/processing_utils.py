...
def sample_target_content_bbox(im, target_bb, search_area_factor,output_sz):
    bbox_all=[]
    for idx in range(len(im)):
        if not isinstance(target_bb[idx], list):
            x, y, w, h = target_bb[idx].tolist()
        else:
            x, y, w, h = target_bb[idx]
        # Crop image
        ...
        x1_pad = max(0, -x1)
        x2_pad = max(x2 - im[idx].shape[1] + 1, 0)

        y1_pad = max(0, -y1)
        y2_pad = max(y2 - im[idx].shape[0] + 1, 0)

        bbox=[x1_pad,y1_pad,crop_sz-x2_pad,crop_sz-y2_pad]#[x1,y1,x2,y2]
        resize_factor = output_sz / crop_sz
        bbox=[i*resize_factor for i in bbox]
        bbox_all.append(bbox)
    return bbox_all #region with content in the padded patch
...
def transform_image_to_crop(box_in: torch.Tensor, box_extract: torch.Tensor, resize_factor: float,
    ...
    box_out_tl = (box_out_center - 0.5 * box_out_wh).clamp(0, crop_sz[0])
    box_out_br = (box_out_center + 0.5 * box_out_wh).clamp(0, crop_sz[0])
    box_out_wh = box_out_br - box_out_tl

    # box_out = torch.cat((box_out_center - 0.5 * box_out_wh, box_out_wh))
    box_out = torch.cat((box_out_tl, box_out_wh))
    if normalize:
